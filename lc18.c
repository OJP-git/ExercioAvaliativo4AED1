...
*/
// { 1,2,3,4,5,6 }
//   a b     c d  target > soma? b++  
//  b -> ou  <- c target < soma? c--
//				  b+1==c || c-1 == b? target > soma? a++ ou target < soma? d--
// { 1,2,3,4,5,6 }
//   a   b c   d 
//  a -> ou  <- d
			...
			while ( d > a + 2 ) {
				if ( d < numsSize - 1 && nums [ d ] == nums [ d + 1 ] ) {
					...
					while ( b < c ) {
						...
						soma = ( long long ) nums [ a ] + nums [ b ] + nums [ c ] + nums [ d ];
						...
						if ( soma < target ) {
							...
							if ( pB != NULL && nums [ b ] == * pB ) {
								b++;
							} else if ( pC != NULL && nums [ c ] == * pC ) {
								c--;
							} else {
								//incio da parte com ajuda de IA
									if ( ( * returnSize ) == capacity ) {
										capacity *= 2;
										returnedMatriz = realloc( returnedMatriz, capacity * sizeof ( int * ) );
